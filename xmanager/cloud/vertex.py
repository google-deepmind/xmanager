...
_TESLA_GPU_TYPES = frozenset({
    xm.ResourceType.P4,
    xm.ResourceType.T4,
    xm.ResourceType.P100,
    xm.ResourceType.V100,
...
def aip_v1_gpu_accelerator_type_str(gpu_type: xm.GpuType) -> str:
  match gpu_type:
    ...
    case _:
      raise ValueError(
          f'Unsupported GPU type {gpu_type}. Supported types are:'
          f' {list(xm.GpuType)}'
      )
...
def get_machine_spec(job: xm.Job) -> Dict[str, Any]:
  ...
  if accelerator and accelerator == aip_v1.AcceleratorType.NVIDIA_TESLA_A100:
    ...
    required_cpus = requirements.task_requirements.get(
        xm.ResourceType.CPU, None
    )

    def _l4_machine_fits(gpus: int, cpus: int) -> bool:
      # G2 machine types bundle a fixed number of L4 GPUs, so the GPU count
      # must match exactly. CPUs are also fixed per machine type; pick the
      # smallest machine that provides at least the requested amount.
      return required_gpus == gpus and (
          required_cpus is None or required_cpus <= cpus
      )

    l4_candidates = [
        (machine_type, (gpus, cpus))
        for (gpus, cpus), machine_type in _L4_GPUS_TO_MACHINE_TYPE.items()
        if _l4_machine_fits(gpus, cpus)
    ]

    if not l4_candidates:
      cpu_str = f' with {int(required_cpus)} CPUs' if required_cpus else ''
      raise ValueError(
          f'l4={required_gpus}{cpu_str} does not fit in any valid machine type.'
      )

    # Pick the smallest machine that satisfies the requirements, ordering by
    # (gpus, cpus).
