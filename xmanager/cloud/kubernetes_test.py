...
class KubernetesTest(unittest.TestCase):
  ...
  def test_resources_from_executor(self):
    executor = local_executors.Kubernetes(
        resources=xm.JobRequirements(cpu=1, ram=1 * xm.GiB))
    requirements = kubernetes.resources_from_executor(executor).to_dict()
    self.assertDictEqual(requirements['limits'], {
        'cpu': '1',
        'memory': str(2**30),
    })

  def test_resources_from_executor_gpu(self):
    executor = local_executors.Kubernetes(resources=xm.JobRequirements(v100=4))
    requirements = kubernetes.resources_from_executor(executor).to_dict()
    self.assertDictEqual(requirements['limits'], {'nvidia.com/gpu': '4'})

  def test_resources_from_executor_empty(self):
    executor = local_executors.Kubernetes(resources=xm.JobRequirements())
    requirements = kubernetes.resources_from_executor(executor).to_dict()
    self.assertDictEqual(requirements['limits'], {})
