...
_DEFAULT_XMANAGER_UI_URL = 'https://ui.alpha.example.com'
...
class XManagerCloudExperiment(xm.Experiment):
  ...
  def create_artifact(
    ...
    Args:
      ...
      acls: ACLs to control access to the artifact. The artifact inherits ACLs
        from the parent XID. The ACLs per artifact are the merged ACLs of its
        parent XID(s) and the ACLs specified in this field, e.g. any writer of
        the parent XID can also write this artifact.
      tags: Tags for the artifact.
      notes: Notes for the artifact.
      lifecycle_phase: The lifecycle phase of the artifact. This defines the
        phase of the experiment lifecycle that the artifact is associated with.
      data_type: The type of data the artifact represents.
      mime_type: The MIME type of the artifact, if applicable.
      additional_info: Additional info to display for the artifact.
...
def _generate_create_experiment_request(
    ...
    # TODO: (internal issue) - Remove/update default value (e.g. to just the author).
...
class XManagerCloudWorkUnit(xm.WorkUnit):
  ...
  async def _launch_job_group(
    ...
    # TODO: (internal issue) - call WaitOperation once implemented and get the full
...
def _create_artifact(
  ...
  Args:
    ...
    acls: ACLs to control access to the artifact. The artifact inherits ACLs
      from the parent XID. The ACLs per artifact are the merged ACLs of its
      parent XID(s) and the ACLs specified in this field, e.g. any writer of the
      parent XID can also write this artifact.
    tags: Tags for the artifact.
    notes: Notes for the artifact.
    lifecycle_phase: The lifecycle phase of the artifact. This defines the phase
      of the experiment lifecycle that the artifact is associated with.
    data_type: The type of data the artifact represents.
    mime_type: The MIME type of the artifact, if applicable.
    additional_info: Additional info for the artifact.

  Returns:
    The created artifact.
  """
  return stub.create_artifact(
      experiment_state_service_pb2.CreateArtifactRequest(
